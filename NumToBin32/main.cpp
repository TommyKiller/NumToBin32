...
void FloatToBin(long double value)
{
	...
	printf(
		...
		Integer part: %s				\n\
		Float part = (%f - %.0f) * %.0f;\n\
		Float part: %s					\n", value, bs, p, value_int_part.to_string().c_str(), value, pow(2, bs), pow(2, float_part_bits), value_float_part.to_string().c_str());

	printf(
		"\		Result: %s				\n\
		Hex result: %X					\n", value32ieee754.to_string().c_str(), value32ieee754);
